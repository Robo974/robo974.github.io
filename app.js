...
window.onload = function(){
    ...
    var show = document.getElementById("btn-show-all");      //Show All Videos
    var search = document.getElementById("btn-search");      //Search Bar
    
    var search_word = document.getElementById("txt-search"); //Key Word
    var result = document.getElementById("results");         //Result Page
    var son_show = document.createElement("p");
    var full = false;     //Whether all videos are showed
    var found = false;    //Whether the video that matches is found

    show.onclick = function(){
        search_word.value = "";   //Clear search bar
        if(full === false){
            son_show.innerHTML = "";         //Clear all results
            for(var i = 0; i < 10; i++){
                son_show.innerHTML += generateRowHTML(videosList[i]);
                result.appendChild(son_show);
            }
            full = true;     //All videos are showed
        }
        else{
            alert("All the videos have been shown already.");
        }
    }
    search.onclick = function(){
        son_show.innerHTML = "";        //Clear all results
        if(search_word.value === ""){   //If no keyword is entered
            alert("Please enter a key word.");
        }
        else{
            for(var i = 0; i < 10; i++){               //Determine whether the description contains keywords
                if(videosList[i].description.indexOf(search_word.value.toLowerCase()) != -1){    //ignore case 
                    son_show.innerHTML += generateRowHTML(videosList[i]);
                    result.appendChild(son_show);
                    found = true;
                }
            ...
            if(found === false){
                son_show.innerHTML = `No videos found with the keyword "${search_word.value}"`;
            }
        }
        found = false;
        full = false;
